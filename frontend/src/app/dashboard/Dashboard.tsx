...
export function CustomerDashboard() {
  ...
  const stats = [
    { label: 'Copy đã tạo', value: '312', icon: FileText, color: 'bg-primary/50', change: '+8 tuần này' },
    { label: 'Quota còn lại', value: '188', icon: Zap, color: 'bg-warning/100', change: '/ 500 tháng này' },
    { label: 'Model đang dùng', value: 'GPT-4o', icon: Brain, color: 'bg-primary/50', change: 'Fine-tuned sẵn sàng' },
    { label: 'Chất lượng TB', value: '91%', icon: Star, color: 'bg-warning/100', change: 'Top 15%' },
  ];
  ...
  const quickActions = [
    ...
    { title: 'Fine-tuning Studio', desc: 'Huấn luyện AI theo giọng văn thương hiệu riêng', icon: Brain, path: '/fine-tune', color: 'from-green-500 to-green-600', cta: 'Khám phá' },
    ...
    { title: 'Kiểm Tra Đạo Văn', desc: 'Kiểm tra tính độc đáo nội dung AI trước khi xuất bản', icon: Key, path: '/plagiarism-check', color: 'from-green-500 to-green-600', cta: 'Kiểm tra' },
  ];
    ...
    <Layout>
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-1">Chào {user?.name?.split(' ').pop()} 👋</h1>
            <p className="text-foreground/70">Sẵn sàng tạo copy marketing đỉnh cao hôm nay?</p>
          </div>
        ...
        <Card className="p-5 mb-8 bg-gradient-to-r from-amber-50 to-green-50 border-amber-200">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="font-semibold text-foreground">Quota tháng 3/2026</p>
              <p className="text-xs text-foreground/70">312 / 500 copy đã dùng</p>
            </div>
            <Badge className="bg-warning/15 text-amber-800 border-0">62.4%</Badge>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-foreground">Copy tạo trong tuần</h3>
              <Badge className="bg-primary/10 text-primary border-0">+12% so với tuần trước</Badge>
            </div>
            <AreaChart
              data={WEEKLY_DATA}
              xKey="day"
          ...
          <Card className="p-5">
            ...
            <div className="space-y-3">
              {RECENT_COPIES.map((copy, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-lg hover:bg-surface-muted transition-colors cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-1">
                      <span className="text-xs text-muted-foreground/80">{copy.time}</span>
        ...
        <Card className="p-5 mt-6">
          <h3 className="font-semibold text-foreground mb-4">Model AI khả dụng</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { name: 'GPT-4o', status: 'online', latency: '~2s', color: 'bg-primary/50', badge: 'Mặc định' },
              { name: 'GPT-3.5', status: 'online', latency: '~0.8s', color: 'bg-primary/50', badge: 'Nhanh' },
              { name: 'Llama 3.1 70B', status: 'online', latency: '~3s', color: 'bg-warning/100', badge: 'Open-source' },
              { name: 'Fine-tuned E-com', status: 'ready', latency: '~1.5s', color: 'bg-primary/50', badge: 'Custom' },
            ].map(m => (
              <div key={m.name} className="flex items-center gap-2.5 p-3 bg-surface-muted rounded-lg border">
                <div className={`w-2 h-2 rounded-full ${m.color} flex-shrink-0`} />
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-foreground truncate">{m.name}</p>
                  <p className="text-xs text-muted-foreground">{m.latency}</p>
                </div>
              </div>
            ))}
          </div>
