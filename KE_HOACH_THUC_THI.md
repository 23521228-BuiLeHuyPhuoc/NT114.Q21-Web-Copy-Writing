...
- [ ] **Trang Thư Viện Templates (`/templates`):** Lưới (Grid) chứa các thẻ prompt dùng sẵn, phân loại theo trường `category` của template.
...
- [ ] **Bảng Quản lý Data (`/admin/users`, `/admin/contents`, `/admin/templates`, `/admin/generate-options/industries`, `/admin/generate-options/copy-types`, `/admin/generate-options/tones`, `/admin/plans`, `/admin/payments`, `/admin/models`):** Cung cấp bộ lưới CRUD đầy đủ.
...
- [ ] **`Templates` Collection:** `name`, `description`, `category`, `systemPrompt` (Chứa biến `{{var}}`), `isSystem` (Boolean), `authorId`.
- [ ] **`GenerateOption` Collection:** `group`, `name`, `slug`, `description`, `icon`, `color`, `order`, `isActive` cho ngành nghề, loại nội dung và tone giọng văn trong Generate.
